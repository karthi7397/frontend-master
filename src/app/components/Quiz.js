...
export default function Quiz({ quizType, user, resetQuiz }) {
  ...
  const finishQuiz = () => {
    ...
    const currentUserId = localStorage.getItem('frontend-master_quizUserId');

    if (currentUserId) {
      // Get existing quiz results from local storage
      const storedResults = JSON.parse(localStorage.getItem('frontend-master_quizResults') || '[]');
      ...
      localStorage.setItem('frontend-master_quizResults', JSON.stringify(updatedResults));
    }
  };
