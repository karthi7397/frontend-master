...
export default function UserSelection({ selectUser }) {
  ...
  useEffect(() => {
    ...
    const storedResults = localStorage.getItem('frontend-master_quizResults');
    if (storedResults) {
  ...
  const addUser = (name) => {
    ...
    localStorage.setItem('frontend-master_quizUserId', newUserId);
    ...
    localStorage.setItem('frontend-master_quizResults', JSON.stringify(updatedUsers));
